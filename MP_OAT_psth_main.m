function [] = MP_OAT_psth_main()
    %% Get directory with all animals and their data
    original_path = 'C:\Users\mahan\OneDrive\Documents\MATLAB\Powers OAT\Animals Refined'; %hard coded to save time, should be: uigetdir(pwd);
    start_time = tic;
    ...
    for animal = 1:length(animal_names)
        ...
        if config.ignore_animal
            ...
            if config.parse_files
                ...
                parsed_path = MP_OAT_parser(animal_path, animal_name, config.total_trials, ...
            ...
            if config.create_psth
                psth_path = OAT_batch_format_psth(parsed_path, animal_name, config);
            else
