function [] = lda_classifier_main(varargin)
    %% Get data directory
    project_path = get_project_path(varargin);
    start_time = tic;

    %% Import psth config and removes ignored animals
    config = import_config(project_path, 'lda_classifier');
    ...
    for dir_i = 1:length(dir_list)
        ...
        [classifier_path, classifier_failed_path] = create_dir(psth_path, 'lda_classifier');
        export_params(classifier_path, 'lda_classifier', config);
        try
            %% Check to make sure paths exist for analysis and create save path
            ...
            batch_lda_classify(project_path, dir_save_path, dir_failed_path, ...
