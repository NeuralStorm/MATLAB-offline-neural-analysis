function sepdata = cal_sep_analysis(filename_meta, sep_map, sep_window, ...
    ...
    background_window = find((time_vec >= (- abs(config.baseline_window_start)) * 1000) ...
        & (time_vec <= (- abs(config.baseline_window_end) * 1000)));
    ...
    for i = 1:size(sep,2)
        ...
        posthreshbackground = background + (config.standard_deviation_coefficient * background_sd);
        negthreshbackground = background - (config.standard_deviation_coefficient * background_sd);
