function [] = graph_sep(sub_rows, sub_cols)
    ...
    for file_i = 1:length(file_list)
        ...
        for region_i = 1:length(unique_regions)
            ...
            %% Save fig
            % [region_path, ~] = create_dir(figure_path, region);
            filename = [filename_meta.filename, '_', region, '.fig'];
            savefig(gcf, fullfile(figure_path, filename));
        end
    end
end