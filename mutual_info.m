function [] = mutual_info(psth_path)
    ...
    mi_path = strcat(psth_path, '/mutual_info');
    if ~exist(mi_path, 'dir')
       mkdir(psth_path, 'mutual_info');
    end

    for file_path = 1:length(psth_files)
        % Load file
        ...
        %% Saving the file
        mi_filename = strrep(filename, 'PSTH', 'mutual');
        mi_filename = strrep(mi_filename, 'format', 'info');
        matfile = fullfile(mi_path, [mi_filename, '.mat']);
