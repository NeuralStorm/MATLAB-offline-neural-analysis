function [parsed_path] = MP_OAT_parser(dir_path, animal_name, total_trials, total_events, trial_lower_bound, ...
    ...
    % *.plx
    [parsed_path, failed_path] = create_dir(dir_path, 'parsed');
    [plx_files] = get_file_list(dir_path, '.plx', ignore_sessions);
    [mat_files] = get_file_list(dir_path, '.mat', ignore_sessions); %added specifically for Marissa Powers' Obsticle Avoidance
    ...
    for file_index = 1: length(plx_files)
        ...
        try
            ...
            %% Specific code for building event timestamps for Marissa Powers Obstical Avoidance 
             % load .mat file associated with plx
