function [] = continuous_sep_main()
    project_path = uigetdir(pwd);
    start_time = tic;

    %% Import psth config and removes ignored animals
    config = import_config(project_path, 'continuous_sep');
    config(config.include_dir == 0, :) = [];

    %% Creating paths to do analog analysis
    [continuous_sep_path, continuous_failed_path] = create_dir(project_path, 'continuous_sep');
    export_params(continuous_sep_path, 'continuous_sep', config);
    ...
    for dir_i = 1:length(dir_list)
        ...
        if dir_config.filter_data
            [filter_data_path, filter_failed_path] = create_dir(continuous_sep_path, 'filtered_data');
        ...
        if dir_config.create_sep
            [sep_path, ~] = create_dir(continuous_sep_path, 'sep');
            ...
            try
                ...
                if dir_config.use_raw
                    ...
                    filter_path = [continuous_sep_path, '/filtered_data'];
        ...
        if dir_config.sep_analysis
            sep_path = [continuous_sep_path, '/sep'];
