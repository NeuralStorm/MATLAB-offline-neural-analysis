function [color_struct, region_list] = create_color_struct(color_map, label_log)
    [tot_colors, ~] = size(color_map);
    %% Find powers and regions in feature
    region_list = unique(label_log.chan_group);
    ...
    for region_i = 1:numel(region_list)
        ...
        tot_region_chans = numel(unique(label_log.channel(strcmpi(label_log.chan_group, region))));
        color_struct.(region).tot_region_chans = tot_region_chans;
        color_struct.(region).indices = find(ismember(label_log.chan_group, region));
        %% set region color
