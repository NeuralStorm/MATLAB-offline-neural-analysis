function [pca_results, pc_log] = calc_power_pca(label_log, mnts_struct, ...
    ...
    for region_index = 1:length(unique_regions)
        ...
        if strcmpi(feature_filter, 'pcs')
            ...
            if feature_value > tot_pcs
                ...
                pc_log.(region) = pc_log.(region)(1:tot_pcs, :);
            end
        ...
        elseif strcmpi(feature_filter, 'percent_var')
            ...
            pca_score = (mnts_struct.(region).(mnts_type) - estimated_mean) ...
                * coeff(:,1:tot_pcs);
            pc_log.(region) = pc_log.(region)(1:tot_pcs, :);
        end
        ...
        %% Reset labeled data
        pc_log.(region).sig_channels = pc_names;
    end
end