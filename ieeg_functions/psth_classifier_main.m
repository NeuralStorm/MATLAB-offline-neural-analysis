function [] = psth_classifier_main(varargin)
    %% Get data directory
    project_path = get_project_path(varargin);
    start_time = tic;

    %% Import psth config and removes ignored animals
    config = import_config(project_path, 'bootstrap_classifier');
    ...
    for dir_i = 1:length(dir_list)
        ...
        try
            ...
            batch_classify_template(project_path, dir_save_path, dir_failed_path, ...
