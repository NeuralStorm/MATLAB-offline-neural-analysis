...
bk2=binno;%138  %%usu 400 for 500ms before and 500ms after
...
figure(m);
        ...
        bar(Sneurons1(m,:));
        hold on;
axis([xLL xUL yLL yUL])
                  line(X,Y);        line(X,Y2); line(X,Y3);
axis([xLL xUL yLL yUL])
title('Tilt 1')
            
        subplot(422);hold off;
        bar(Sneurons2(m,:));
axis([xLL xUL yLL yUL])
        line(X,Y);        line(X,Y2); line(X,Y3);
axis([xLL xUL yLL yUL])
