...
savdir='H:\Tilt Data\RAVI\RAVI mat formatted\';
...
for i=1:tiltnum
    ...
    %Names
    Events(i).name=tilt_names{i};
    Events(i+6).name=[tilt_names{i},'_background'];   %names for background events
    
    %Channels
    Events(i).channel=i;
    Events(i+6).channel=i+6;
    
    %Timestamps
    ts_temp=Events(i).ts(1:4:end);
    Events(i).ts=[];
    Events(i).ts=ts_temp;
    Events(i+6).ts=ts_temp-1; %subtracting 1 second for background event ts
