function [outdataset,popPSTH,popPEM]=Rec_Field_Analysis(filenames,options)
...
for f=1:length(filenames)
    ...
    [d,f]=fileparts(options.filename);
    clc
    fprintf('File: %s \n',f)
                    ...
                    if options.RM==0
                        ...
                        if isempty(PEHM)~=1
                            if isfield(options,'TRM')
                                ...
                                if options.TRM
                                    [NormRFMatrix2D]=receptive_field_TRM(PEHM,options);
                                else
                            ...
                            elseif isfield(options,'Obs')
                                if options.Obs
                                    [NormRFMatrix2D]=receptive_field_Obs(PEHM,options);
                                end
                            elseif isfield(options,'WB')
                                if options.WB
                                    [NormRFMatrix2D]=receptive_field_WB(PEHM,options);
                                end
...
function [NormRFMatrix2D]=receptive_field_TRM(PEHM,options)

NormRFMatrix2D = recfield_TRM(PEHM,options);
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,10),1);
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,10),2);
NormRFMatrix2D(:,10)=[];
...
NormRFMatrix2D = recfield_TILT(PEHM,options);
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,12),1);%changed from 10
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,12),2);
NormRFMatrix2D(:,12)=[];
NormRFMatrix2D(:,end+1)=NormRFMatrix2D(:,2)-NormRFMatrix2D(:,7).*1/options.bin.*...
    (NormRFMatrix2D(:,5)-NormRFMatrix2D(:,4));
NormRFMatrix2D(:,end+1)=NormRFMatrix2D(:,3)-NormRFMatrix2D(:,7);

function [NormRFMatrix2D]=receptive_field_Obs(PEHM,options)

NormRFMatrix2D = recfield_Obs(PEHM,options);
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,12),1);%changed from 10 to 12
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,12),2);%10->12
NormRFMatrix2D(:,12)=[]; %10->12
NormRFMatrix2D(:,end+1)=NormRFMatrix2D(:,2)-NormRFMatrix2D(:,7).*1/options.bin.*...
    (NormRFMatrix2D(:,5)-NormRFMatrix2D(:,4));
NormRFMatrix2D(:,end+1)=NormRFMatrix2D(:,3)-NormRFMatrix2D(:,7);
function [NormRFMatrix2D]=receptive_field_WB(PEHM,options)

NormRFMatrix2D = recfield_WB(PEHM,options);
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,10),1);
NormRFMatrix2D(:,end+1)=options.anchan(NormRFMatrix2D(:,10),2);
NormRFMatrix2D(:,10)=[];
NormRFMatrix2D(:,end+1)=NormRFMatrix2D(:,2)-NormRFMatrix2D(:,7).*1/options.bin.*...
...
if options.RM==0
    tempdataset=dataset({NormRFMatrix2D,'NormResponse',' Response',...
        ...
        'BackgroundAvg','BackgroundSTD','RMB_sub','Duration_s','LocNum',...
        'PRFNum','RFS','Channel','Unit','RM_BS','Peak_BS' },{repmat({options.regionname},...
        rows,1),'RegionOrElectrode'},{repmat(options.bin,rows,1),'Binsize'}...
        ,fileinfos{:},{repmat(max(options.timewindow),rows,1),'Timewindow'}); %%added in RMB_sub and Duration(s)
    
else
    tempdataset=dataset({NormRFMatrix2D,'NormResponse',' Response',...
        ' Peak','PeakLatency','RM_BS','Peak_BS','BackgroundAvg','BackgroundSTD', 'RMB_sub','Duration_s',...
