...
%         PEHMBackground = PEHM(1:(options.pretime/options.bin),j,options.backgroundevent);%i,6 for speed testing on 071312; 15 for SLOW 5, 10, 15 tilts.  16 for SPEED tilts; 8? so all background is from before all start?
        PEHMBackground = PEHM(1:(options.pretime/options.bin),j,options.backgroundevent);%i,6 for speed testing on 071312; 15 for SLOW 5, 10, 15 tilts.  16 for SPEED tilts; 8? so all background is from before all start?
...
%%%%Jaimie experimenting 73012
        CurrMatrixS=slideAverage(CurrMatrix,10);%changed from 3 to 10 on 9/30/13
        PEHMBackgroundS=slideAverage(PEHMBackground,10);%%trying to fix my troubles   PEHMBackgroundT???
...
%             PEHMBackground = cat(1,PEHMBackground,CurrMatrix) ;
    ...
    %%again, jaimie screwing wtih stuff 112512
        ...
        NSTD =3;%3; % Anitha used 2.58 for 99% confidence, 3 for Foffani's Paper, higher for more stringent
...
% % % % %         AboveThreshold=CurrMatrix(Index1:Index2);
        ...
        y=find(x<20);%5 with a 2ms bin for no gaps >10ms,  changed 093013 to 20ms for fun testing
        z=diff(y);
        zz=find(z>1);
...
%             BfrSpikesPerBin = 0 ;
        ...
        elseif length(AboveThreshold)<3%%origincally 3 changed by jaimie 08012013
