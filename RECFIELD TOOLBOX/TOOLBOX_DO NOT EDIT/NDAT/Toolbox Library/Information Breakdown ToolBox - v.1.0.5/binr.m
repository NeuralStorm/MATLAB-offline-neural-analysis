function R = binr(R, nt, nb, opt, par)
...
switch opt
    case {'eqpop'; 'eqspace'; 'ceqspace'; 'gseqspace'}
        isBuiltinFunc = true;
    otherwise
        isBuiltinFunc = false;
end
...
for k=1:L
    ...
    if isBuiltinFunc && nargin==5
        % Built-in function are allowed a more elaborate input:
        edges = edgesFunc(R(k,mask(:)), nb, par);
    else
        edges = edgesFunc(R(k,mask(:)), nb);
        
        if length(edges)~=nb+1
    ...
    [ignore, R(k,mask(:))] = histc(R(k,mask(:)), edges);
end
