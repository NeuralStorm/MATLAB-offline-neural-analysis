function [] = crude_synergy_redundancy(original_path, classified_path, animal_name)
    % Grabs all the csv files
    csv_mat_path = [original_path, '/*.csv'];
    csv_files = dir(csv_mat_path);
    for file = 1: length(csv_files)
        if contains(csv_files(file).name, '_unit')
            ...
            % neuron_labels(1, :) = [];
        elseif contains(csv_files(file).name, '_population')
            spreadsheet_table = readtable(csv_files(1).name);
        end
    end
    ...
    % unit_index scales the unit spreadsheet for the info
    % Starts at 2 to ignore the row names in the table
    unit_index = 2;
    for pop = 1:length(pop_files)
        ...
        for neuron = 1:total_neurons
            % (unit20msspreadsheet.neuron_corrected_info(2,1) > 0) && (unit20msspreadsheet.neuron_label(2,1) == 'Indirect')
        ...
        direct_syn_red = corrected_pop_info - direct_unit_corrected_sum;
        indirect_syn_red = corrected_pop_info - indirect_unit_corrected_sum;
        classified_struct.direct_syn_red = direct_syn_red;
        classified_struct.indirect_syn_red = indirect_syn_red;
        % TODO what happens when syn red is 0?
        classified_struct.direct_syn_red_bool = direct_syn_red > 0;
        classified_struct.indirect_syn_red_bool = indirect_syn_red > 0;
