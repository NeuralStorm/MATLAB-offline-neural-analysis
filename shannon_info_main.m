function [] = shannon_info_main(varargin)
    ...
    for dir_i = 1:length(dir_list)
        ...
        e_msg_1 = 'No data directory to find rr';
        [info_path, info_failed_path] = create_dir(psth_path, 'shannon_info');
        export_params(info_path, 'shannon_info', config);
        try
            %% Check to make sure paths exist for analysis and create save path
            e_msg_2 = ['No ', curr_dir, ' rr data for shannon mutual info'];
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%    Shannon Info Analysis   %%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            batch_info(project_path, dir_save_path, dir_failed_path, ...
