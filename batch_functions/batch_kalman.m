function [] = batch_kalman(kalman_path, psth_path, config)
    ...
    for file_index = 1:length(file_list)
        ...
        region_state = init_neural_state(psth_struct, event_ts, config.trial_range);
        ...
        init_kalman(event_ts, measurements, region_state, config.pre_time, config.post_time, config.bin_size, config.training_size)
        calc_kalman_coeff(measurements, event_ts, psth_struct, labeled_data, config.training_size, config.pre_time, config.post_time, config.bin_size)


        %% Saving the file
        matfile = fullfile(closed_path, [psth_filename, '.mat']);
        save(matfile, 'region_state', 'labeled_data', 'psth_struct');
